...
async def get_task_by_id(owner_id: UUID, db: AsyncSession, task_id: UUID) -> TaskRead:
    repository = TaskRepository(db)
...
async def get_important_task(
    owner_id: UUID, db: AsyncSession, project_id: UUID
) -> TaskRead | str:
    tasks_db = await get_tasks_by_project(owner_id, db, project_id)
...
async def create_task(owner_id: UUID, db: AsyncSession, task: TaskCreate) -> TaskRead:
    await get_project(owner_id, db, task.project_id)
...
async def get_all_tasks(owner_id: UUID, db: AsyncSession) -> list[TaskRead]:
    repository = TaskRepository(db)
...
async def show_tasks(
    ...
    if project_id is None and task_id is None:
        return await get_all_tasks(owner_id, db)

    if project_id is not None and task_id is None:
        await get_project(owner_id, db, project_id)
        return await get_tasks_by_project(owner_id, db, project_id)
    ...
    if (
        ...
        return [await get_task_by_id(owner_id, db, task_id)]

    return []


async def update_task(
    owner_id: UUID, db: AsyncSession, task_id: UUID, updated_task: TaskUpdate
) -> TaskRead:
    if updated_task.project_id:
        await get_project(owner_id, db, updated_task.project_id)
...
async def delete_task(owner_id: UUID, db: AsyncSession, task_id: UUID) -> None:
    await get_task_by_id(owner_id, db, task_id)
